...
public class SpecialPrizesController implements Initializable {

    RedeemPage special = new RedeemPage();
    ...
    @FXML
    public Button redeem1300, redeem2500;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        special.setPane(homePane, redeemPane, storePane, mePane);
        special.setImage(homeImage, redeemImage, storeImage, meImage);
        special.setLabel(homeLabel, redeemLabel, storeLabel, meLabel);
    }

    public void mouseClick(MouseEvent mouseEvent) throws IOException {
        special.mouseClick(mouseEvent);
    }

    public void mouseEnter(MouseEvent mouseEvent) throws IOException {
        special.mouseEnter(mouseEvent);
    }

    public void mouseExit(MouseEvent mouseEvent) throws IOException {
        special.mouseExit(mouseEvent);
    }
    ...
    public void redeem(ActionEvent actionEvent) throws IOException {
        ...
        yes.setOnAction(e->{
            if(Controller1.scores>=100) {
                try {
                    if(actionEvent.getSource()==redeem1300) {
                        Controller1.scores -= 1300;
                        ...
                        dialog1.show();
                    }else if(actionEvent.getSource()==redeem2500) {
                        Controller1.scores -= 2500;
                        String point = Integer.toString(Controller1.scores);
                        FileOutputStream output = new FileOutputStream("file.txt");
                        output.write(point.getBytes());
                        JFXDialogLayout dialogLayout1 = new JFXDialogLayout();
